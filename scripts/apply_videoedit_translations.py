#!/usr/bin/env python3
"""menu.videoEdit + videoedit.* anahtarlarını i18n/videoedit_bundles.json ile tüm .lproj dosyalarına yazar."""
...
BUNDLES_PATH = ROOT / "i18n" / "videoedit_bundles.json"

_LINE_RE = re.compile(r'^"(menu\.videoEdit|videoedit\.[^"]+)"\s*=\s*"(.*)";\s*$')
...
def parse_videoedit_keys(path: Path) -> dict[str, str]:
    out: dict[str, str] = {}
...
def main() -> int:
    ...
    tr_strings = parse_videoedit_keys(tr_path)
    ...
    for lproj in sorted(COMET.glob("*.lproj")):
        ...
        if n:
            print(f"{lang}: updated {n} videoedit keys")
        total += n
