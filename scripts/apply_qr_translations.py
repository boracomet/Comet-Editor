#!/usr/bin/env python3
"""qr.* anahtarlarını i18n/qr_bundles.json + tr.lproj kurallarıyla tüm .lproj dosyalarına yazar."""
...
BUNDLES_PATH = ROOT / "i18n" / "qr_bundles.json"

_LINE_RE = re.compile(r'^"(qr\.[^"]+)"\s*=\s*"(.*)";\s*$')
...
def parse_qr_keys(path: Path) -> dict[str, str]:
    out: dict[str, str] = {}
...
def main() -> int:
    ...
    tr_strings = parse_qr_keys(tr_path)
    ...
    for lproj in sorted(COMET.glob("*.lproj")):
        ...
        if n:
            print(f"{lang}: updated {n} qr keys")
        total += n
